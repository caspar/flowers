...
let hueSlider, saturationSlider, saturationDropoffSlider, brightnessSlider
let fHue, fSaturation, fSaturationDropoff, fBrightness
let hue, saturation, saturationdropoff, brightness
...
function setup(){
  ...
  fHue = createDiv();
  fHue.class("valueDisplay");
  hueSlider = createSlider(1, 360, 340, 1);
  hueSlider.class("Slider");

  fSaturation = createDiv();
  fSaturation.class("valueDisplay");
  saturationSlider = createSlider(1, 100, 100, 1);
  saturationSlider.class("Slider");

  fSaturationDropoff = createDiv();
  fSaturationDropoff.class("valueDisplay");
  saturationDropoffSlider = createSlider(-5, 2, 1, 0.01);
  saturationDropoffSlider.class("Slider");

  fBrightness = createDiv();
  fBrightness.class("valueDisplay");
  brightnessSlider = createSlider(1, 100, 100, 1);
...
function draw(){
  ...
  saturationDropoff = saturationDropoffSlider.value();
  ...
  for(let theta = 0; theta < v.length; theta++){
    fill(hue, saturation-theta*saturationDropoff, brightness);
  ...
  fHue.html("Hue: " + hueSlider.value());
  fSaturation.html("Saturation: " + saturationSlider.value());
  fSaturationDropoff.html("Gradient:" + saturationDropoffSlider.value());
  fBrightness.html("Brightness: " + brightnessSlider.value());
  
  petalNum.html("Number of petals: " + pNumSlider.value());
